...
* https://www.elastic.co/guide/en/logstash/current/dir-layout.html[directorie Layout - logstash]

* https://github.com/rossiya/pihole-docker-logstash/tree/master/logstash/pipeline[example - github]

* https://www.elastic.co/guide/en/logstash/current/logstash-settings-file.html[logstash.yml - not used]

* https://www.elastic.co/guide/en/logstash/current/multiple-pipelines.html[pipelines.yml]

* https://www.elastic.co/guide/en/logstash/current/pipeline.html[pipline - Input - Filter - Output]
...
[NOTE]
The output ist stored in the ``htmlcov`` folder. Open the ``index.html`` for an overview of your test coverage.

For writing tests see the https://flask.palletsprojects.com/en/1.1.x/tutorial/tests/[flask tutorial].


== todo

.logstash read via Websocket? 

* https://www.elastic.co/guide/en/logstash/current/input-plugins.html[Link]
* https://www.elastic.co/guide/en/logstash/current/plugins-inputs-websocket.html[Websocket]


 add .env Versions to Dokerfile

 bug in htmlcov mv --> rysnc and delete is also an option
